...
```text
A forest of exquisitely fine vertical luminous spectral lines of varying height rising from a baseline, drawn as hairlines. Read left to right as increasing frequency: the envelope decays smoothly along a power-law slope, so the low-frequency end at the left carries the most energy. MOST of the lines are dim teal and blend into a noisy thicket. Only a small regularly spaced subset stands taller and sharper, lit in hot coral, as though selected, occurring at several points along the slope. A smooth faint curve sweeps through the thicket at the level of the noise.
```
